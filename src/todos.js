...
class TodoReduceStore extends ReduceStore {
    getInitialState() {
        return {
            todos: [
                {id: id(), content: 'Learn flux', completed: false},
                {id: id(), content: 'Learn redux', completed: false},
                {id: id(), content: 'Drink beer', completed: true},
            ],
            showCompleted: true,
        };
    }
    ...
    reducer(state, action) {
        ...
        switch (type) {
            ...
            case COMPLETE_TODO:
                newState = {...state, ...{todos: [...state.todos]}};
                const idx = newState.todos.findIndex(t => t.id === payload.id);
                newState.todos[idx] = {
                    ...state.todos[idx],
...
document.forms.newTaks.addEventListener('submit', e => {
    e.preventDefault();
...
const templateTodoComponent = ({id, content, completed}) => {
    return `
        <section>
            ...
            <input id="el-${id}" name="todoCheckbox" ${
        completed ? 'checked' : ''
        } data-taskid="${id}" type="chekbox"> 
        </section>`;
};
...
const render = () => {
    ...
    if (todoReduceStore.isHistory()) {
        ...
        undoBtn.textContent = undoBtnText + '-' + todoReduceStore.__historylength;
    } else {
